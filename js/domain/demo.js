...
export function loadDemoData() {
  ...
  const students = NAMES.map((n, i) => {
    ...
    return newStudent({
      ...
      level: pick(LEVELS, i),
      group: pick(GROUPS, i),
      // Alguns grups tenen cotutoria i alguns casos, tutoria individualitzada.
      tutors: i % 4 === 0 ? [pick(TUTORS, i), pick(TUTORS, i + 2)] : [pick(TUTORS, i)],
      tutorIndividual: i % 5 === 0 ? pick(TUTORS, i + 1) : '',
  ...
  students.forEach((s, i) => {
    ...
    for (let k = 0; k < count; k += 1) {
      ...
      records.push(newRecord({
        ...
        participants: type === 'entrevistaFamilia' ? [`Mare de ${s.name}`, s.tutors[0]] : [s.tutors[0]],
